...
namespace RVO {
	...
	int KdTree::QueryNearByAgents(const Vector2 &pos, int* ptr, int currentCount, int totalCount, Fix16 range, int addId) const
	{
		...
		QueryNearByAgents(pos, ptr, currentCount, totalCount, range * range, 0, addId);
		return currentCount;
	}
	...
	void KdTree::QueryNearByAgents(const Vector2 &pos, int* ptr, int& addCount, int totalCount, Fix16 rangeSq, size_t node, int addId) const
	{
		if (agentTree_[node].end - agentTree_[node].begin <= MAX_LEAF_SIZE) {
			...
			if (distSqLeft < distSqRight) {
				if (distSqLeft < rangeSq) {
					QueryNearByAgents(pos, ptr, addCount, totalCount, rangeSq, agentTree_[node].left, addId);

					if (distSqRight < rangeSq) {
						QueryNearByAgents(pos, ptr, addCount, totalCount, rangeSq, agentTree_[node].right, addId);
					}
				}
			}
			else {
